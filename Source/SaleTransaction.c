...
short readMagCard(TransactionMsgStruc *transMsg)
{
 ...
 // strncpy((char *)transMsg->Track2Data,TRACK2_DATA/*transMsg->CardDetails.track*/,strlen(TRACK2_DATA)/*strlen(transMsg->CardDetails.track)*/);        //35 Hardcoded Track2 data 
  strncpy((char *)transMsg->Track2Data,transMsg->CardDetails.track,strlen(transMsg->CardDetails.track));        //35 for dynamic mag swipe //YS
 // strncpy(track2data,TRACK2_DATA,strlen(TRACK2_DATA));        //35 Sample Track 2 Data provided by Client 
  strncpy(track2data,transMsg->CardDetails.track,strlen(transMsg->CardDetails.track));        //35 Sample Track 2 Data provided by Client  //YS
	//strcpy(transMsg->CardDetails.track, "6220151105029518=20051011000045800100");
	//strcpy((char *)transMsg->Track2Data, transMsg->CardDetails.track);
	//strcpy(track2data, transMsg->CardDetails.track);
...
int swipeOrEMVCardEntry(TransactionMsgStruc *transMsg)
{
	...
	if( hMagReader == -1)
	{
		...
		if(LOG_STATUS == LOG_ENABLE)
		{
			LOG_PRINTF (("magnetic reader open = %d",hMagReader));	
		}
	}
	...
	window(1,1,30,20);
	clrscr ();
	write_at("CUSTOMER",8,11,8);
if(transMsg->TrTypeFlag == PIN_VALIDATE_MSG_TYPE_CASE)
{
  ...
  //unsigned char reqBuff[PAYMENT_REQUEST_SIZE]={0};//96 -REQUEST BUFFER
	...
	short recvLength = 0;
		...
		field_struct iso8583_field_table[] =
		{
			...
			//{ 37 + SKIP, 8, BCD_STR, (void *)BitmapStructOb.discard, sizeof(BitmapStructOb.discard) },
			//{ 38 + SKIP, 8, BCD_STR, (void *)BitmapStructOb.discard, sizeof(BitmapStructOb.discard) },
			//{ 39 + SKIP, 8, BCD_STR, (void *)BitmapStructOb.discard, sizeof(BitmapStructOb.discard) },
	...
	if(retVal == _SUCCESS )//checking for response buffer size 
	{
		...
		if(!strcmp((char *)BitmapStructOb.field_39,"00"))
		{
			...
			if(LOG_STATUS == LOG_ENABLE)
			{
				LOG_PRINTF(("Sale Transaction successful\n"));    
			}
		}
